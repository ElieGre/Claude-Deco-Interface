// One agent's live Claude session: chat items, permission prompts, model/effort/mode, token usage.
// Every agent tab owns one of these stores (store/agents.ts).
...
/** A /btw side question: answered from the conversation so far, never added to it. */
...
export interface SessionState {
  ...
  items: ChatItem[]
  /** A resumed conversation's transcript is still loading (the chat shows a skeleton, not the empty state). */
  restoring: boolean
  permissions: PermissionRequest[]
  /** Project-relative paths attached to the next message as @mentions */
  attachments: string[]
  sideQuestions: SideQuestion[]
  /** Extra working directories added with /add-dir (they last for this session only) */
  extraDirs: string[]
  ...
  dismissSide(id?: string): void
  addDirectory(dir: string): Promise<boolean>
...
export function createSessionStore(): SessionStore {
  return createStore<SessionState>()((set, get) => {
    ...
    const control = async (fn: (key: string) => Promise<void>): Promise<boolean> => {
      const { key, status } = get()
      if (!key || status === 'closed') return false
      try {
        await fn(key)
        return true
      } catch (err) {
    ...
    const patchSide = (id: string, patch: Partial<SideQuestion>) =>
      set((s) => ({ sideQuestions: s.sideQuestions.map((q) => (q.id === id ? { ...q, ...patch } : q)) }))
    ...
    return {
      ...
      async start({ resume } = {}) {
        ...
        const { prefs } = get()
        set({ ...fresh(), key, sessionId, status: 'starting', restoring: !!resume, model: prefs.model ?? null, effort: prefs.effort ?? null })

        if (resume) {
          try {
            const transcript = await window.api.history.messages(resume, cwd)
            if (get().key === key) set({ items: itemsFromTranscript(transcript), restoring: false })
          } catch (err) {
            if (get().key === key) set({ restoring: false })
      ...
      async askSide(question) {
        const { key, status } = get()
        const id = uid('btw')
        set((s) => ({ sideQuestions: [...s.sideQuestions, { id, question, pending: true }] }))
        if (!key || status === 'closed') return patchSide(id, { pending: false, error: 'No live session — send a message first.' })
        try {
          const r = await window.api.claude.sideQuestion(key, question)
          patchSide(id, { pending: false, answer: r?.response ?? 'No answer.' })
        } catch (err) {
          patchSide(id, { pending: false, error: String(err instanceof Error ? err.message : err) })
        }
      },

      dismissSide(id) {
        set((s) => ({ sideQuestions: id ? s.sideQuestions.filter((q) => q.id !== id) : s.sideQuestions.slice(0, -1) }))
      },

      async addDirectory(dir) {
        const dirs = [...new Set([...get().extraDirs, dir])]
        const ok = await control((key) => window.api.claude.setAdditionalDirectories(key, dirs))
        if (ok) set({ extraDirs: dirs })
        return ok
      },
